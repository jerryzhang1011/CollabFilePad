...
} from "./chunk-ZVCAJHIR.js";

// node_modules/y-textarea/dist/y-textArea.es.js
...
var events = ["keyup", "mouseup", "touchstart", "paste", "cut", "selectend"];
var Cursor = class {
    ...
    const awarenessUpdate = (event) => {
      ...
      for (const [clientID, change] of changes.entries()) {
        ...
        if (encodedStart === void 0 || encodedEnd === void 0)
          continue;
        const start = createAbsolutePositionFromRelativePosition(JSON.parse(encodedStart), doc);
        const end = createAbsolutePositionFromRelativePosition(JSON.parse(encodedEnd), doc);
  ...
  constructor(yText, textField, options) {
    ...
    const yTextObserver = (__event, transaction) => {
      ...
      if (textField.getRootNode().activeElement === textField) {
        const startPos = createAbsolutePositionFromRelativePosition(relPosStart, doc);
        const endPos = createAbsolutePositionFromRelativePosition(relPosEnd, doc);
...
//# sourceMappingURL=y-textarea.js.map
